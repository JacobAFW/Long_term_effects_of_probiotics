...
Ordinations and EnvFit analysis (Vegan package) were used to determine if storage of faecal samples with the OMNIGene collection kit differed from the gold standard freezing at -80. The EnvFit function was applied to an ordination using NMDS that was based on a Bray-Curtis dissimilarity matrix calculated from data normalised with Total Sum Scaling. The significance was based on 10,000 permutations and was transformed based on the Benjamini-Hochberg (BH) procedure.
...
# Subset to storage comparison samples

```{r,warning=F,message=F}
ps.storage_test <- ps2.TSS %>% 
  subset_samples(Storage_test == "Yes")
```
...
plot_ordination(filtered_ps, ps_ordination, color = variable, 
  ...
  scale_color_hue(labels = c("OMNIGene", "Frozen"))
 }

ordination_plots(ps.storage_test, "Storage", "OMNIGene", "Frozen")
```
