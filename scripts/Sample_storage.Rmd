...
ordination_plots(ps2_storage, "URN", "PCoA", "bray")
...
# homogeneity condition
dist <- vegdist(ps_otu)
homogeneity <- as.data.frame(anova(betadisper(dist, ps_metadata$URN))) %>% 
  tableGrob() %>% 
...
ggsave("outputs/envfit.png", dpi = 600, height = 5, width = 7, grid_plot)
```
...
**We do not have enough biological replicates to perform [DE analysis](https://europepmc.org/article/PMC/4878611)**.

#### Export Plot
```{r,warning=F,message=F,eval=F}
grid_plot <- grid.arrange(plot_bar_auto(ps.top20, "Phylum"),
             plot_bar_auto(ps.top20, "Genus"),
             nrow = 2, heights = c(1, 1))
...
#### Export plot
```{r, warning=F, message=F}
ggsave("outputs/aplha_plot.png", dpi = 600 , height = 8, width = 8)
```
